...
public class UserController {
	...
	public ResponseMessage<EditUser> get(@PathVariable("_id") Integer _id) {        
        return new ResponseMessage<EditUser>(HttpStatus.OK.value(), "User fetched successfully.", new EditUser(userRepository.findById(_id).get()));
    }
	...
	public ResponseMessage<Users> add(@RequestBody Users users) {
		users.set_id(userService.nextId());
	...
	public ResponseMessage<String> delete(@PathVariable("_id") Integer _id) {
		String message = "";
		try {
			Users users = userRepository.findById(_id).get();
